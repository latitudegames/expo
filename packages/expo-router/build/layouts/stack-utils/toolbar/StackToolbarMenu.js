...
const StackToolbarMenu = ({ children, ...props }) => {
    ...
    if (placement === 'bottom') {
        ...
        return (<bottom_toolbar_native_elements_1.NativeToolbarMenu {...rest} image={image ?? loadedImage ?? undefined} children={validChildren}/>);
    }
...
const StackToolbarMenuAction = (props) => {
    ...
    if (placement === 'bottom') {
        const { icon, image, ...rest } = props;
        return <bottom_toolbar_native_elements_1.NativeToolbarMenuAction {...rest} image={image ?? loadedImage}/>;
    }
